...
InfAdmixture::InfAdmixture(my_rng * rng,Data *d,State *s,double alpha,int v,double initialq,bool test)
{
  this->rng=rng;
	atest=test;
	data=d;
...
{
  int r1=RandomInteger(rng,0,P[a].size()-1),r2=r1;
  while(r2==r1) r2=RandomInteger(rng,0,P[a].size()-1);
	double x=rnd(rng);// the proportion of r1 to move to r2
	...
	logstateprob=state->admixtureLogLikelihood(&Q,&P,&QcolSums);
	if(log(rnd(rng))>logstateprob + priorForP(a) - oldloglik - oldpriorP) {
		//rejected
		P[a][r1]=op1;	
		P[a][r2]=op2;
...
int InfAdmixture::moveQrow(int a)
{
  int r1=RandomInteger(rng,0,Q[a].size()-1),r2=r1;
  while(r2==r1) r2=RandomInteger(rng,0,Q[a].size()-1);
	double x=rnd(rng);// the proportion of r1+r2 to move to r2
...
//	cout<<"p="<<logstateprob<<" + "<<priorForQ(a)<<" - "<<oldloglik<<" - "<<oldpriorQ<<endl;
	if(log(rnd(rng))>newloglik + priorForQ(a) - oldloglik - oldpriorQ) {
		//rejected
//		cout<<"rejected"<<endl;
		Q[a][r1]=oq1;
		Q[a][r2]=oq2;
