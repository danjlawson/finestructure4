...
{
public:
    Inf1(my_rng * rng,State *s,int datainference=INFDATA_COUNTS,int v=0,int tmax=1500,int treescale=0);///<Constructor if everything already known. 
    Inf1(my_rng * rng,State *s,string newick,int v=0);///<Constructor if tree already known.
    //    Inf1(my_rng * rng,Data *activedata,int initpop=-1,int datainference=INFDATA_COUNTS,int modeltype=1,int v=0,int tmax=1500);///<Constructor from data only.  Starts with one population or N populations
    ...
    void exportXmlHead(std::ostream * fileout,std::string fs,std::string type,long x);///<Xml start
    ...
    inline vector<int> ignoreIgnorables(vector<int> plist){
	  ...
	  if(state->getIgnore(plist[counter])) plist.erase(plist.begin()+counter);
	  else counter++;
      }
	...
	if(state->getIgnore(plist[c1])) found=true;//return(true);
      }
