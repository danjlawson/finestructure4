...
namespace fines
{
  ...
  Data::Data(string filename,int ignore,bool yhead,bool xhead,bool square)
{
    datacfactor=-1;
    ...
    if(!file.good()) {
      cerr<<"Cannot open chunkcount file "<<filename<<endl;
	...
	if(line.substr(0,8).compare(string("#Cfactor"))==0) {
	  ...
	  if(isheader) {// read the names
		  ...
		  if(!square){///< The header refers to columns, not individual samples
		    colnames=names;
		    names.clear();
		  }
	    lineon++;continue;
	  ...
	  {//read the row of data
		  ...
		  {//read each element of the matrix
			...
			names.push_back(dval.c_str());
		      ...
		      if(square && (indrowon>=(int)names.size() || indrowon<0)) throw(string("Invalid data. Possibly there are names in the data file without RECIPIENT in the top left... you should modify the data file to ChromoPainter format, but you might be able to get around this with the -X and -Y flags."));
		      if(square && names[indrowon].compare(dval.c_str())!=0){
			...
			cerr<<"Writing data to "<<tfilename<<" check it manually!"<<endl;
			cerr<<"This is the symptom if you have the wrong line endings."<<endl<<"Try converting them to that of your platform (or UNIX are usually handled correctly everywhere)"<<endl;
			cerr<<"Alternatively, you may not be using a square matrix."<<endl;
			output(tfilename);			
...
//	if((int)names.size()<i) names.push_back(itoa(i));
  ...
  Data::Data(std::vector < std::vector<double> > * inmat,std::vector <std::string> namesin,std::vector <std::string> colnamesin)
{
    numignore=0;
    ...
    n=data.size();
    names=namesin;
    if(colnamesin.size()!=0){
      square=false;
      colnames=colnamesin;
    }
    testData();
    // I don't know why I did it the long way:
...
{
    (*out)<<"NAME, ";
    for (int i=0;i<(int)k-1;i++)(*out)<<getcolnames(i)<<", ";
    (*out)<<getcolnames(k-1)<<endl;
    for (unsigned int i=0;i<data.size();i++)
    {
...
// sort into descending order so that removing is safe
  ...
  for(unsigned int i=1;i<ind.size();i++){
    for(unsigned int c1=0;c1<data.size();c1++){
      ...
      if(square) data[c1][ind[0]]+=data[c1][ind[i]];
    }
...
//  cout<<endl;
  for(unsigned int i=ind.size()-1;i>0;i--){
    if(square) for(unsigned int c1=0;c1<data.size();c1++) data[c1].erase(data[c1].begin() + ind[i]);
