...
namespace fines
{
  ...
  Inf1::Inf1(my_rng * rng,State *s,int datainference,int v,int tmax,int treescale)
{
  this->rng=rng;
	warned=false;
	test_max=tmax;
...
//	reorderSuper(state);
	...
	currentheight=0;
	nodes=vector<Node*>(state->getDim()  + state->getDim() - state->numIgnore()-1);
	for (unsigned int i=0;i<nodes.size();i++) {
	  nodes[i]=new Node();
	  nodes[i]->setId(i);
	}
	root=nodes[state->getDim()  + state->getDim() - state->numIgnore()-2];
	createSkeletonTree();
	if(verbose) state->setprint(&cout);
	//if(verbose) {cout<<"Printing Data"<<endl;data->print(&cout);}
}

  Inf1::Inf1(my_rng * rng,State *s,string newick,int v)
{
  this->rng=rng;
  	warned=false;
	test_max=0;
    ...
    root = new Node(s->getActiveData(),&newick,NULL,&nodes);
...
//   Inf1::Inf1(my_rng * rng,Data *activedata,int initpop,int datainference,int modeltype,int v,int tmax)
// {
//   this->rng=rng;
//   warned=false;
//   test_max=tmax;
// 	verbose=(v==1);
// 	silent=0;if(v>1)silent=1;
// 	if(initpop<0 || initpop>activedata->getDim()) initpop=activedata->getDim();
// 	state=new State(rng,data,initpop,vector<double>(1.0),1.0,BETAMOD_CONST,2.0,dlength,dref,-1,modeltype);///**** dref
...
{
	if(xml) *fileout<<"<Tree>";
	*fileout<<root->newick(state->getActiveData())<<";";
	if(xml) *fileout<<"</Tree>";
	*fileout<<endl;
}
...
{
	if((int)mat->size()!=state->getDim()) {cerr<<"setcertainty: wrong matrix size"<<endl;;throw(string("setcertainty: wrong matrix size!"));}
...
{// puts all the super individuals at the start
  ...
  for(int c1=0;c1<in->getP();c1++) {
	  if(testIgnorePop(c1)) {
	    if(nfound<c1) {
		  if(verbose) cout<<"Reordering "<<c1<<" ("<<state->getnames(in->getIndInPop(c1)[0])<<") and "<<nfound<<" ("<<state->getnames(in->getIndInPop(nfound)[0])<<")"<<endl;
		  in->reorderPop(c1,nfound);
		  nfound++;
	    }
	  }else if(verbose)cout<<"Number"<<c1<<" ("<<state->getnames(in->getIndInPop(c1)[0])<<") ok!"<<endl;
	  if(verbose) {
		for(int i=0;i<=c1;i++) cout<<state->getnames(in->getIndInPop(i)[0])<<",";
	  	cout<<endl;
	  }
  }
}
...
cout<<"Done with indivs"<<endl;*/
	...
	for(int c1=1;c1<in->getP();c1++) {
	  ...
	  if(!inFullOrder(in->getIndInPop(npos)[0],in->getIndInPop(c1)[0])) {
	    while(npos>=0) {
		  if(verbose) cout<<"Comparing "<<c1<<" ("<<state->getnames(in->getIndInPop(c1)[0])<<") and "<<npos<<" ("<<state->getnames(in->getIndInPop(npos)[0])<<")"<<endl;
		  if(!inFullOrder(in->getIndInPop(c1)[0],in->getIndInPop(npos)[0])) {
			  npos++;break;
		  }else npos--;
	    }
	    if(npos<c1 && npos>=0) {
		  if(verbose) cout<<"Reordering "<<c1<<" ("<<state->getnames(in->getIndInPop(c1)[0])<<") and "<<npos<<" ("<<state->getnames(in->getIndInPop(npos)[0])<<")"<<endl;
...
//  cout<<"MERGEHILLCLIMB MOD="<<treemodification<<" stopattop="<<stopattop<<endl;
	...
	long c1=0;
	long numints=0;
	if(!stopattop) numints=state->getP()-1-state->numIgnore();
	initheight=-INFINITY;
	...
	while(stillgoing) {
		...
		if(state->getP()==state->numIgnore()+1) stillgoing=false;
		if(verbose) {
		cout<<"Accepted"<<endl;
		state->setprint(&cout);
		...
		if(treemodification==2) doHyperParameterMaximisation(1000);
	}
...
{
	bool stillgoing=true;
	if(state->getP()==state->getDim()-state->numIgnore()) stillgoing=false;
	while(stillgoing) {
		stillgoing=doBestSplit(stopattop);
		if(state->getP()==state->getDim()) stillgoing=false;
...
void Inf1::doHyperParameterMaximisation(int numsteps){
  InfMCMC tmcmc(rng,state,INFDATA_COUNTS,0,false);
...
// construct a list of merges to consider
	if(mmax<0 || (state->getP()-state->numIgnore())*(state->getP()-state->numIgnore()-1)/2 < mmax) {
	  ...
	  if(!warned) {warned=true;
		cerr<<"WARNING!  NOT TESTING ALL "<<state->getP()*(state->getP()-1)/2<<" COMBINATIONS! (max "<<mmax<<")"<<endl;
	  }
	  for(int i=0;i<mmax;i++) {
		int a=0,b=0;
		while(a==b) {
		  a=RandomInteger(rng,state->numIgnore(),state->getP()-1);
		  b=RandomInteger(rng,state->numIgnore(),state->getP()-1);
...
// test the merges to find the best
	...
	for(unsigned int i=0;i<li.size();i++) {
		...
		string s1=state->getnames(state->getIndInPop(li[i])[0]);
		string s2=state->getnames(state->getIndInPop(lj[i])[0]);
...
// test the splits to find the best
  ...
  void Inf1::exportXmlHead(std::ostream * fileout,std::string fs,std::string type,long x)
{
  ...
  *fileout<<"<datafiles>"<<state->getFileNames()<<"</datafiles>"<<endl;
...
int Inf1::whichPopNode(int i)
{
  ...
  if(verbose) cout<<"looking for population "<<i<<endl;
  for(unsigned int c1=0;c1<nodes.size();c1++){
    if(verbose) cout<<"Node "<<c1<<endl;
