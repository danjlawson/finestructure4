...
#include "ChromoPainterRecmap.h"
 ...
 ///* A data type in which all the forward backward details are stored
  ...
  double InitialiseForward(signed char * newh, signed char ** existing_h, double * prev, double * MutProb_vec, int *p_Nhaps,int *p_Nloci, double * copy_probSTART, double * TransProb); // Initialise the forward algorithm (fills normalized prev column, returns logScale[0])

  double forwardAlgorithm(signed char * newh, signed char ** existing_h, float ** ahat, double * MutProb_vec, int *p_Nhaps,int *p_Nloci,double * copy_prob, double * copy_probSTART, double * TransProb, double * logScale, struct param_t *Par);  // Perform the forward algorithm inference (linear-rescale: float ahat + double logScale)
